...
cabinApp.displayCabin = function (cabin) {
    $('#result').append('\n    <div class=\'cabinImg\'>\n        <h1 id="cabinTitle">' + cabin.title + '<h1>\n        <img src=\'' + cabin.src + '\' alt=\'' + cabin.alt + '\'>\n    </div>\n    ');
};

cabinApp.choice = function (userInput1, userInput2) {
...
cabinApp.init = function () {
    cabinApp.canvasSize();
    $('#cabinSearch').on('submit', function (e) {
        e.preventDefault();
        $('#result').empty();
        $('#instruction').fadeOut();
        $('button').fadeIn();
        ...
        $('input[type=text]').val("");
        var cabinSize = $('input[name=size]:checked').val();
        var unique = $.parseJSON($('input[name=uniqueness]:checked').val());
    ...
    $('#name').on('submit', function (e) {
        ...
        var newTitle = $('input[type=text]').val();
        $('#cabinTitle').empty();
        $('#cabinTitle').append(newTitle);
    });
};
